<template>
    <div>
        <div class="editor-wrapper bg-base-300 border-l-2 border-primary p-3 my-2" :class="{ 'editable-tiptap': tiptapIsEditable }">
            <div v-if="editor">
                <div v-if="tiptapIsEditable"  class="editor-buttons bg-slate-300 dark:bg-slate-800 flex items-center py-1 flex-wrap">
                    <!-- Bold -->                
...
<script setup lang="ts">
    ...
    const { tiptapIsEditable } = toRefs(props)
    const emit = defineEmits(['update:modelValue'])  
    //const isEditable = ref(false)
    //isEditable.value = tiptapIsEditable.value
   ...
   //})
    ...
    const editor = useEditor({
        ...
        editable: tiptapIsEditable.value,
        extensions: [
            StarterKit,
            Link,
            TextAlign.configure({
    ...
    watch(tiptapIsEditable, () => {       
        editor.value.setEditable(tiptapIsEditable.value)
        console.log(tiptapIsEditable.value)
    })
...
<style scoped>
    ...
    .editable-tiptap {
        @apply border border-blue-100 dark:border-slate-800 rounded
    }
</style>